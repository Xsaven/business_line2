...
<!--            <v-bottom-action :task="task" />-->
          <v-get-task-report v-if="(task.report_type === 'quiz' || task.report_type === 'star_quiz' || task.report_type === 'download_file') && (Number(user.direction_id) !== Number(task.direction_id) || task.finished)" status="no" :report="task_report"/>
