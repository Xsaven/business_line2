...
<!--            <v-bottom-action :task="task" />-->
          ...
          <v-get-task-report v-else-if="(task.report_type === 'quiz' || task.report_type === 'star_quiz' || task.report_type === 'download_file') && (Number(user.direction_id) !== Number(task.direction_id) || task.finished)" status="no"/>
          <v-my-report v-else-if="Number(user.direction_id) !== Number(task.direction_id) || task.finished" :task="task" :reports="reports" :report="task.finished ? task_report : undefined"/>
