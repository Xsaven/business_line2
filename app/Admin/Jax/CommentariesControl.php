...
//            }
            lte_log_success('Одобрил комментарий '.$comment->id);
    ...
    public function drop(int $id)
    {
        ...
        if ($comment && $comment->delete()) {
            if ($comment->commentaryRoom instanceof CommentaryRoom && $comment->commentaryRoom->id === 1) {
                AllUserExec::dispatch(['v-home-commentaries:add_to_child' => [$comment->id]]);
                lte_log_success('Удалил комментарий '.$comment->id);
            } else {
    ...
    public function switch_moderation()
    {
        ...
        lte_log_success(
            (AppServiceProvider::$cfg['free_chat'] ? 'Выключил' : 'Включил').' модерацию комментариев'
        );
    ...
    public function switch_commentary()
    {
        ...
        lte_log_success(
            (AppServiceProvider::$cfg['osm'] ? 'Выключил' : 'Включил').' режим магазина'
        );
    ...
    public function switch_support()
    {
        ...
        lte_log_success(
            ($val ? 'Включил' : 'Выключил').' поддержку'
        );
    ...
    public function switch_stickers()
    {
        ...
        lte_log_success(
            ($val ? 'Включил' : 'Выключил').' стикеры в чате'
        );
